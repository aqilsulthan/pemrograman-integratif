...
func AddBook(bookData model.Book) model.Book {
    ...
    // Auto generate id menggunakan uuid
...
func GetBook(bookId string) (int, model.Book) {
    ...
    // membuat variabel book untuk menyimpan data buku berdasarkan id
    ...
    // Close() akan dipanggil jika data dari database sudah didapatkan
    defer row.Close()

    // untuk setiap baris data
    for row.Next() {
        // masukkan berbagai atribut data buku seperti judul dll ke dalam variabel book
        ...
        // jika tidak ada error, tampilkan data buku
        case nil:
            log.Println(book)
    ...
    // mengembalikan angka 1 sebagai tanda data ditemukan dan data buku yang ditemukan
    return 1, book
...
func GetBooks() []model.Book {
    ...
    // close dipanggil jika seluruh data berhasil diambil
    defer rows.Close()

    // membuat variabel books untuk menampung berbagai data buku
    var books []model.Book = []model.Book{}

    // untuk setiap data
    for rows.Next() {
        // membuat variabel book untuk menyimpan sebuah data buku
        var book model.Book = model.Book{}
    ...
    // jika jumlah data di dalam books sama dengan 0, maka data kosong
    if len(books) == 0 {
...
func DeleteBook(id string) bool {
    ...
    // jika terdapat error, tampilkan pesan error nilai false dikembalikan
    if err != nil {
